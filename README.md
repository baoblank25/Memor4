# Memor4 — Every Conversation Remembered
...
[Features](#features) • [Demo](#demo) • [Installation](#installation) • [Configuration](#configuration) • [Usage](#usage) • [Architecture](#architecture)
...
- **💭 Conversation Memory**: Extracts and stores important facts from conversations
- **🗣️ Voice Interaction**: Natural conversation support with text-to-speech feedback
- **🛡️ Safety Monitoring**: Detects allergens and medication hazards (Overwatch Mode)
- **👨‍👩‍👧‍👦 Caregiver Dashboard**: Web interface for family members to manage profiles and memories
- **📊 Memory Analytics**: Track conversations, memories, and interaction patterns
...
├── overshoot-sentinel.js   # Safety monitoring module
└── faces/                  # Stored face images for recognition
```
...
└──────────┘                     └──────────┘
     ...
     │                                 └─→ Overshoot SDK (Hazards)
     │
     └─→ Browser APIs (Camera, Mic, SpeechRecognition)
```
...
└── static/
    ...
    ├── overshoot-sentinel.js # Safety module
    └── faces/               # Face images
```
...
**Made with ❤️ for families affected by memory conditions**

[⬆ Back to Top](#memor4--every-conversation-remembered)
